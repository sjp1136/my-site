...
async function monitor(root, meta, scannedProject, options, pluginMeta, targetFileRelativePath) {
    ...
    if (package_managers_1.GRAPH_SUPPORTED_PACKAGE_MANAGERS.includes(packageManager)) {
        ...
        if (monitorGraphSupportedRes.ok) {
            return await monitorGraph(root, meta, scannedProject, pluginMeta, targetFileRelativePath);
        }
...
async function monitorGraph(root, meta, scannedProject, pluginMeta, targetFileRelativePath) {
    const packageManager = meta.packageManager;
